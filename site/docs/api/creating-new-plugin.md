...
!!! note
    ...
    See [here](/api/indexing/#supported-plugins) for the list of currently available plugins.
...
- [Indexing Flow](#indexing-flow)
    * Interfaces for defining a new index - implementations of [Index](/api/scaladoc/1.2.0/io/xskipper/index/index.html) along with [IndexFactory](/api/scaladoc/1.2.0/io/xskipper/index/IndexFactory.html).
    * Interface for specifying how to store the metadata in the metadatastore - implementations of [MetaDataTranslator](/api/scaladoc/1.2.0/io/xskipper/metadatastore/MetaDataTranslator.html).

- [Query Evaluation Flow](#query-evaluation-flow) 
    * Interfaces for defining how query expressions are mapped to abstract conditions on metadata - implementations of [MetaDataFilter](/api/scaladoc/1.2.0/io/xskipper/search/filters/MetadataFilter.html) along with [MetaDataFilterFactory](/api/scaladoc/1.2.0/io/xskipper/search/filters/MetadataFilterFactory.html).
    * Interfaces for translating an abstract clause to a specific implementation according to the metadatastore - Implementations of [ClauseTranslator](/api/scaladoc/1.2.0/io/xskipper/metadatastore/ClauseTranslator.html).
...
To use a plugin, load the relevant implementations using the Registration module ([Scala](/api/scaladoc/1.2.0/io/xskipper/Registration$.html), [Python](/api/pythondoc/1.2.0/html/index.html#module-xskipper.registration)).   
For example:
...
For the full list of plugins see [here](/api/indexing/#plugins).
...
*Implementation(s) of [MetaDataType](/api/scaladoc/1.2.0/io/xskipper/index/metadata/MetadataType.html)*
...
*Implementation(s) of [Index](/api/scaladoc/1.2.0/io/xskipper/index/index.html) along with [IndexFactory](/api/scaladoc/1.2.0/io/xskipper/index/IndexFactory.html)*

Support for new indexes can be achieved by implementing a new class that implements the [Index](/api/scaladoc/1.2.0/io/xskipper/index/index.html) abstract class.  
...
Along with the Index you need to define an [IndexFactory](/api/scaladoc/1.2.0/io/xskipper/index/IndexFactory.html) - 
...
*Implementation(s) of [MetaDataTranslator](/api/scaladoc/1.2.0/io/xskipper/metadatastore/MetaDataTranslator.html)*

!!! info
    Xskipper uses by default Parquet as the metadatastore.    
    The Parquet metadatastore stores the metadata for the objects as rows in parquet files (for more details see [here](/api/developer/parquet-metadatastore-spec/)).      
...
For example, the MinMax index translates its values to a nested row with `min` and `max` values (see [here](https://github.com/xskipper-io/xskipper/blob/9573e1de24e3ccc7856e10a4004a6e09710b16b4/src/main/scala/io/xskipper/metadatastore/parquet/ParquetBaseMetaDataTranslator.scala#L28))
...
To do so you have 2 options:
    ...
    * Use the default java serialization provided by the parquet metadata store. To do so you need to define the UDT and register it. For example, for bloom filter, we use the following definition to get the default java serialization:
    ...
    Then register the UDT to Spark using the [ParquetMetadataStoreUDTRegistration](/api/scaladoc/1.2.0/org/apache/spark/sql/types/ParquetMetadataStoreUDTRegistration$.html) object:
    ```Scala
    ...
    !!! note
        ...
        A recommended pattern is to define and register the UDT in the [Clause Translator](#defining-translation-for-the-clause) object where you also define the Clause Translation logic.    
        This object will be loaded when [registering](/api/creating-new-plugin/#registering-existing-plugins) the Clause Translator.
...
*Implementations of [Clause](/api/scaladoc/1.2.0/io/xskipper/search/clause/Clause.html)*
...
*Implementation(s) of [MetaDataFilter](/api/scaladoc/1.2.0/io/xskipper/search/filters/MetadataFilter.html) along with [MetaDataFilterFactory](/api/scaladoc/1.2.0/io/xskipper/search/filters/MetadataFilterFactory.html)*

The filter processes the query tree and labels it with clauses. In most cases we would like to map expressions to clauses.
Therefore, xskipper provides a basic implementation of a filter called [BaseMetadataFilter](https://github.com/xskipper-io/xskipper/blob/0b430e9df3abaec2ee8ec530c702b6534c9f751f/src/main/scala/io/xskipper/search/filters/MetadataFilter.scala#L26)
which processes the query tree automatically for AND and OR operators, leaving the user to handle only the remaining expressions. Implementations which extend the [BaseMetadataFilter](https://github.com/xskipper-io/xskipper/blob/0b430e9df3abaec2ee8ec530c702b6534c9f751f/src/main/scala/io/xskipper/search/filters/MetadataFilter.scala#L26) need only specify how expressions are mapped to clauses.   
...
Along with a Filter you need to define a [MetadataFilterFactory](/api/scaladoc/1.2.0/io/xskipper/search/filters/MetadataFilterFactory.html). The MetadataFilterFactory specifies which filters should run given the available indexes.
For example, see the [RegexIndexFactory](https://github.ibm.com/HRL-IOT/xskipper-regex-plugin/blob/master/src/main/scala/io/xskipper/plugins/regex/index/RegexIndexFactory.scala).

### Define translation for the clause

*Implementations of [ClauseTranslator](/api/scaladoc/1.2.0/io/xskipper/metadatastore/ClauseTranslator.html)*
...
For example, for the BloomFilter index which we serializes its metadata, we use a UDF to check whether the given value exists in the metadata or not (see [here](https://github.com/xskippe-io/xskipper/blob/0b430e9df3abaec2ee8ec530c702b6534c9f751f/src/main/scala/io/xskipper/metadatastore/parquet/ParquetBaseClauseTranslator.scala#L83)).
