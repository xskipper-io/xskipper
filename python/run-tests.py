...
def test(root_dir, package):
    ...
    test_dir = path.join(python_root_dir, path.join("xskipper", "tests"))
    test_files = [os.path.join(test_dir, f) for f in os.listdir(test_dir)
                  if os.path.isfile(os.path.join(test_dir, f)) and
                  f.endswith(".py") and not f.startswith("_")]
    extra_class_path = path.join(python_root_dir, path.join("xskipper", "testing"))
...
def prepare(root_dir):
    ...
    delete_if_exists(os.path.expanduser("~/.ivy2/cache/io.xskipper"))
    delete_if_exists(os.path.expanduser("~/.m2/repository/io/xskipper/"))
